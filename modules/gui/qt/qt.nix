  ...
  flake.modules.nixos.wm = {
    ...
    hj.xdg.config.files = {
      "Kvantum/Base16Kvantum/Base16Kvantum.kvconfig".text =
        import ./_theme-config.nix { inherit self; };
      "Kvantum/Base16Kvantum/Base16Kvantum.svg".text =
        import ./_theme-svg.nix { inherit self; };
      "Kvantum/kvantum.kvconfig".text = import ./_config.nix;
    };

    nixpkgs.overlays = [
      (_final: prev:
        let strip = self.lib.stripDesktopFiles;
        in {
          libsForQt5 = prev.libsForQt5.overrideScope (_qt5final: qt5prev: {
